		...
		<table style="width: 100%; border-collapse: collapse">
			...
			<tbody>
				...
				<tr 
					...
					<td v-for="field in fields" v-bind:style="{textAlign: field.alignment}" v-html="record[field.name]" style="vertical-align: top; white-space: nowrap; overflow:hidden"></td>
				</tr>
				<tr v-if="recordMenu != null && selected != null && selected.id == record.id" class="record-menu-container">
					<td :colspan="fields.length">
						<div style="width: 100%;" class="record-menu">
...
// COMPONENT METHODS
	methods: {
		...
		callMenuAction(action, rec) {
			if (actionRouter) {
				actionRouter.call(action, rec);
			} else {
				console.log("action: " + action + " (No actionRouter present)");
			}
		},
